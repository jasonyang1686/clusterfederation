 ...
 * @author vagrant
 */
public class ClusterFederation2HopQueryC9Step2Test {
	...
	public static void main(String[] args) throws Exception {
		...
		BufferedReader brDrugbankDrug = new BufferedReader(
				new FileReader(new File("/home/vagrant/results/drugbankDrug")));
      String lineDrugbankDrug = brDrugbankDrug.readLine();
      drugbankDrug.add(lineDrugbankDrug);
      
		while(lineDrugbankDrug!=null){
			drugbankDrug.add(lineDrugbankDrug);
			lineDrugbankDrug = brDrugbankDrug.readLine();
		}
		
		
		brDrugbankDrug.close();
		...
		// execute query
					...
					for (int i=0; i<drugbankDrug.size();i++){
						...
						String query ="Prefix dbpedia: <http://dbpedia.org/ontology/> "+
								...
								"SELECT ?InteractionName WHERE "+
								"{ "+
						//		"?Drug rdf:type dbpedia:Drug . "+
						//		"?drugbankDrug owl:sameAs ?Drug . "+
								"?InteractionName drugbank:interactionDrug1 <"+drugbankDrug.get(i)+"> . "+
				...
				//	System.out.println(query);
					...
					while(result.hasNext()){
						...
						Value valueOfX = bindingSet.getValue("InteractionName");
						InteractionName.add(valueOfX.toString());	
						count++;	
					}
					}
					...
					FileOutputStream fosInteractionName= new FileOutputStream("/home/vagrant/results/InteractionName");
					OutputStreamWriter oswInteractionName = new OutputStreamWriter(fosInteractionName);
					BufferedWriter bwInteractionName = new BufferedWriter(oswInteractionName);
					
					for (int i=0;i<InteractionName.size();i++){
						bwInteractionName.write(InteractionName.get(i)+"\n");
		
					}
				  bwInteractionName.close();
